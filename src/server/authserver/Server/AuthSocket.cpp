...
void AuthSocket::_SetVSFields(const std::string& rI)
{
	...
	PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(_build > 15595 ? LOGIN_UPD_MOP_VS : LOGIN_UPD_VS);
...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    else
    {
		...
		stmt = LoginDatabase.GetPreparedStatement(_build > 15595 ? LOGIN_SEL_MOP_LOGONCHALLENGE : LOGIN_SEL_LOGONCHALLENGE);
        stmt->setString(0, _login);
		sLog->outDebug(LOG_FILTER_AUTHSERVER, "Realm build is %s than 15595", _build > 15595 ? "greater" : "less");
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (!memcmp(M.AsByteArray(), lp.M1, 20))
    {
		...
		PreparedStatement *stmt = LoginDatabase.GetPreparedStatement(_build > 15595 ? LOGIN_UPD_MOP_LOGONPROOF : LOGIN_UPD_LOGONPROOF);
    ...
    else
    {
        ...
        if (MaxWrongPassCount > 0)
        {
            ...
            PreparedStatement *stmt = LoginDatabase.GetPreparedStatement(_build > 15595 ? LOGIN_UPD_MOP_FAILEDLOGINS : LOGIN_UPD_FAILEDLOGINS);
            stmt->setString(0, _login);
            LoginDatabase.Execute(stmt);

            stmt = LoginDatabase.GetPreparedStatement(_build > 15595 ? LOGIN_SEL_MOP_FAILEDLOGINS : LOGIN_SEL_FAILEDLOGINS);
            stmt->setString(0, _login);
...
bool AuthSocket::_HandleReconnectChallenge()
{
	...
	PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(_build > 155959 ? LOGIN_SEL_MOP_SESSIONKEY : LOGIN_SEL_SESSIONKEY);
    stmt->setString(0, _login);
    PreparedQueryResult result = LoginDatabase.Query(stmt);
...
bool AuthSocket::_HandleRealmList()
{
    ...
    PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(_build > 15595 ? LOGIN_SEL_ACCOUNT_ID_BY_EMAIL : LOGIN_SEL_ACCOUNT_ID_BY_NAME);
    stmt->setString(0, _login);
    PreparedQueryResult result = LoginDatabase.Query(stmt);
    if (!result)
    {
