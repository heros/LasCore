...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(LOGIN_UPD_LOGONPROOF, "UPDATE account SET sessionkey = ?, last_ip = ?, last_login = NOW(), locale = ?, failed_logins = 0, os = ?, last_build = ? WHERE username = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(LOGIN_SEL_LOGONCHALLENGE, "SELECT a.sha_pass_hash, a.id, a.locked, a.last_ip, aa.gmlevel, a.v, a.s, a.last_build FROM account a LEFT JOIN account_access aa ON (a.id = aa.id) WHERE a.username = ?", CONNECTION_SYNCH)
    ...
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT, "INSERT INTO account(username, sha_pass_hash, sha_email_pass_hash, email, joindate) VALUES(?, ?, ?, ?, NOW())", CONNECTION_ASYNC);
    ...
    // Modifications for MoP
	// LOGIN_SEL_ACCOUNT_BANNED_BY_USERNAME
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_BANNED_BY_EMAIL, "SELECT account.id, email FROM account, account_banned WHERE account.id = account_banned.id AND active = 1 AND email LIKE CONCAT('%%', ?, '%%') GROUP BY account.id", CONNECTION_SYNCH);
    // LOGIN_SEL_SESSIONKEY
    PREPARE_STATEMENT(LOGIN_SEL_MOP_SESSIONKEY, "SELECT a.sessionkey, a.id, aa.gmlevel  FROM account a LEFT JOIN account_access aa ON (a.id = aa.id) WHERE email = ?", CONNECTION_SYNCH)
    // LOGIN_UPD_VS
    PREPARE_STATEMENT(LOGIN_UPD_MOP_VS, "UPDATE account SET v = ?, s = ? WHERE email = ?", CONNECTION_ASYNC)
    // LOGIN_UPD_LOGONPROOF
    PREPARE_STATEMENT(LOGIN_UPD_MOP_LOGONPROOF, "UPDATE account SET sessionkey = ?, last_ip = ?, last_login = NOW(), locale = ?, failed_logins = 0, os = ?, last_build = ? WHERE email = ?", CONNECTION_ASYNC)
    // LOGIN_SEL_LOGONCHALLENGE
    PREPARE_STATEMENT(LOGIN_SEL_MOP_LOGONCHALLENGE, "SELECT a.sha_email_pass_hash, a.id, a.locked, a.last_ip, aa.gmlevel, a.v, a.s, a.last_build FROM account a LEFT JOIN account_access aa ON (a.id = aa.id) WHERE a.email = ?", CONNECTION_SYNCH)
    // LOGIN_UPD_FAILEDLOGINS
    PREPARE_STATEMENT(LOGIN_UPD_MOP_FAILEDLOGINS, "UPDATE account SET failed_logins = failed_logins + 1 WHERE email = ?", CONNECTION_ASYNC)
    // LOGIN_SEL_FAILEDLOGINS
    PREPARE_STATEMENT(LOGIN_SEL_MOP_FAILEDLOGINS, "SELECT id, failed_logins FROM account WHERE email = ?", CONNECTION_SYNCH)
    // LOGIN_SEL_ACCOUNT_ID_BY_NAME
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_ID_BY_EMAIL, "SELECT id FROM account WHERE email = ?", CONNECTION_SYNCH)
    // LOGIN_SEL_ACCOUNT_INFO_BY_NAME
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_INFO_BY_EMAIL, "SELECT id, sessionkey, last_ip, locked, v, s, expansion, mutetime, locale, recruiter, os FROM account WHERE email = ?", CONNECTION_SYNCH);
    // LOGIN_SEL_ACCOUNT_BY_IP
    PREPARE_STATEMENT(LOGIN_SEL_MOP_ACCOUNT_BY_IP, "SELECT id, email FROM account WHERE last_ip = ?", CONNECTION_SYNCH)
    // LOGIN_UPD_USERNAME
    PREPARE_STATEMENT(LOGIN_UPD_EMAIL, "UPDATE account SET v = 0, s = 0, email = ?, sha_email_pass_hash = ? WHERE id = ?", CONNECTION_ASYNC);
    // LOGIN_UPD_PASSWORD
    PREPARE_STATEMENT(LOGIN_UPD_MOP_PASSWORD, "UPDATE account SET v = 0, s = 0, sha_email_pass_hash = ? WHERE id = ?", CONNECTION_ASYNC);
    // LOGIN_GET_ACCOUNT_ID_BY_USERNAME
    PREPARE_STATEMENT(LOGIN_GET_ACCOUNT_ID_BY_EMAIL, "SELECT id FROM account WHERE email = ?", CONNECTION_SYNCH);
    // LOGIN_GET_USERNAME_BY_ID
    PREPARE_STATEMENT(LOGIN_GET_MOP_USERNAME_BY_ID, "SELECT email FROM account WHERE id = ?", CONNECTION_SYNCH);
    // LOGIN_SEL_CHECK_PASSWORD
    PREPARE_STATEMENT(LOGIN_SEL_MOP_CHECK_PASSWORD, "SELECT 1 FROM account WHERE id = ? AND sha_email_pass_hash = ?", CONNECTION_SYNCH);
    // LOGIN_SEL_CHECK_PASSWORD_BY_NAME
    PREPARE_STATEMENT(LOGIN_SEL_CHECK_PASSWORD_BY_EMAIL, "SELECT 1 FROM account WHERE email = ? AND sha_email_pass_hash = ?", CONNECTION_SYNCH);
    // LOGIN_SEL_GM_ACCOUNTS
    PREPARE_STATEMENT(LOGIN_SEL_MOP_GM_ACCOUNTS, "SELECT a.email, aa.gmlevel FROM account a, account_access aa WHERE a.id=aa.id AND aa.gmlevel >= ? AND (aa.realmid = -1 OR aa.realmid = ?)", CONNECTION_SYNCH);
    // LOGIN_SEL_ACCOUNT_INFO
    PREPARE_STATEMENT(LOGIN_SEL_MOP_ACCOUNT_INFO, "SELECT a.email, a.last_ip, aa.gmlevel, a.expansion FROM account a LEFT JOIN account_access aa ON (a.id = aa.id) WHERE a.id = ?", CONNECTION_SYNCH);
    // LOGIN_SEL_ACCOUNT_ACCESS
    PREPARE_STATEMENT(LOGIN_SEL_MOP_ACCOUNT_ACCESS, "SELECT a.id, aa.gmlevel, aa.RealmID FROM account a LEFT JOIN account_access aa ON (a.id = aa.id) WHERE a.email = ?", CONNECTION_SYNCH);
