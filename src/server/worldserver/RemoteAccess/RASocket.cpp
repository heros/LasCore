...
int RASocket::check_access_level(const std::string& user)
{
    ...
    PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_MOP_ACCOUNT_ACCESS);
...
int RASocket::check_password(const std::string& user, const std::string& pass)
{
    ...
    PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_CHECK_PASSWORD_BY_EMAIL);
