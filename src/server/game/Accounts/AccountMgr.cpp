...
namespace AccountMgr
{
    AccountOpResult CreateAccount(std::string username, std::string password, std::string email)
    {
        ...
        stmt->setString(2, CalculateShaPassHash(email, password));
    ...
    AccountOpResult ChangeUsername(uint32 accountId, std::string newUsername, std::string newPassword)
    {
        ...
        stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_EMAIL);
    ...
    AccountOpResult ChangePassword(uint32 accountId, std::string newPassword)
    {
        ...
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_MOP_PASSWORD);
    ...
    {
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_GET_ACCOUNT_ID_BY_EMAIL);
        stmt->setString(0, username);
        PreparedQueryResult result = LoginDatabase.Query(stmt);
    ...
    {
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_GET_MOP_USERNAME_BY_ID);
        stmt->setUInt32(0, accountId);
        PreparedQueryResult result = LoginDatabase.Query(stmt);
    ...
    bool CheckPassword(uint32 accountId, std::string password)
    {
        ...
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_MOP_CHECK_PASSWORD);
        stmt->setUInt32(0, accountId);
        stmt->setString(1, CalculateShaPassHash(username, password));
        PreparedQueryResult result = LoginDatabase.Query(stmt);
