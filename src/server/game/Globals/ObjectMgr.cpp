...
bool ObjectMgr::LoadTrinityStrings(const char* table, int32 min_value, int32 max_value)
{
    ...
    if (!result)
    {
        if (min_value == MIN_las_string_ID)              // error only in case internal strings
    ...
    if (min_value == MIN_las_string_ID)
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u las strings from table %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
    else
...
const char *ObjectMgr::GetTrinityString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        sLog->outError(LOG_FILTER_SQL, "Entry %i not found in `las_string` table.", entry);
    else
