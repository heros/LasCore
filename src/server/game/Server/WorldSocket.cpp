...
m_OutActive(false), m_Seed(static_cast<uint32> (rand32()))
{
...
int WorldSocket::SendPacket(WorldPacket const& pct)
{
    ...
    if (m_Crypt.IsInitialized())
    {
        ...
        header.header[2] = (uint32)((length >> 16) & 0xFF);
        header.header[3] = (uint32)((length >> 24) & 0xFF);
...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_INFO_BY_EMAIL);
