...
void Object::_BuildMovementUpdate(ByteBuffer* data, uint16 flags) const
{
    ...
    data->WriteBit(0);//bit 1
    data->WriteBit(flags & UPDATEFLAG_UNK2);
    data->WriteBit(0);
    data->WriteBit(0);//bit 0
    data->WriteBit(0);//bit 3
    data->WriteBit(0);
    data->WriteBit(flags & UPDATEFLAG_SELF);
    data->WriteBit(flags & UPDATEFLAG_GO_TRANSPORT_POSITION);
    data->WriteBit(flags & UPDATEFLAG_UNKNOWN);
    ...
    // data->WriteBit(flags & UPDATEFLAG_TRANSPORT);//not sure
    ...
    if (flags & UPDATEFLAG_LIVING)
    {
        ...
        data->WriteBit(!(movementFlags & MOVEMENTFLAG_SPLINE_ELEVATION));       // Has spline elevation not sure
        data->WriteBit(!movementFlags);
        data->WriteBit(0);//self->IsSplineEnabled());                                // Has spline data
        ...
        data->WriteBit(!((movementFlags & (MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_FLYING)) || (movementFlagsExtra & MOVEMENTFLAG2_ALWAYS_ALLOW_PITCHING)));       // Has pitch
        data->WriteBit(1);
        ...
        if (self->m_movementInfo.t_guid)
        {
            ...
            data->WriteBit(transGuid[3]);                                                 // Has transport time 2
            data->WriteBit(transGuid[0]);
            data->WriteBit(transGuid[4]);
            data->WriteBit(transGuid[5]);                                                 // Has transport time 3
    ...
    if (flags & UPDATEFLAG_GO_TRANSPORT_POSITION)
    {
        ...
        ObjectGuid transGuid = self->m_movementInfo.t_guid;                                                  // Has GO transport time 3
...
void Object::GetUpdateFieldData(Player const* target, uint32*& flags, bool& isOwner, bool& isItemOwner, bool& hasUnitAll, bool& isPartyMember) const
{
...
void Object::_SetUpdateBits(UpdateMask* updateMask, Player* target) const
{
    ...
    GetUpdateFieldData(target, flags, isOwner, isItemOwner, hasUnitAll, isPartyMember);
    ...
    for (uint16 index = 0; index < valCount; ++index, ++indexes)
        if (_fieldNotifyFlags & flags[index] || (flags[index] & UF_FLAG_UNITALL && hasUnitAll) || (*indexes && IsUpdateFieldVisible(flags[index], isSelf, isOwner, isItemOwner, isPartyMember)))
...
void Object::_SetCreateBits(UpdateMask* updateMask, Player* target) const
{
    ...
    for (uint16 index = 0; index < valCount; ++index, ++value)
        if (_fieldNotifyFlags & flags[index] || (flags[index] & UF_FLAG_UNITALL && hasUnitAll) || (*value && IsUpdateFieldVisible(flags[index], isSelf, isOwner, isItemOwner, isPartyMember)))
            updateMask->SetBit(index);
}
